--这个程序里，字符串现在是根据目录遍历每个txt文档获取第一行知道的，我想改为，是在 "J:\\0911\\分机账号.txt"这个文件里查找，
--这个文件的格式是32401:雷ぃ傷う|冰璐つ筱馨|べ杨。|べ陈。|べ明°|べ黄。，
--冒号前面作为最终复制目标文件夹名字，冒号后面作为寻找的名字字符串集合，用|串接在一起的
--2.0和1.0版本处理的是每个文档只有一组角色名组合
--3.0版本处理的是，所有角色名组合在一个文档里，每行一组
--分发配置文件
local lfs = require("lfs")

-- 定义源目录和目标根目录
local sourceFolder = "D:\\天龙相关\\临时处理\\all"
...
local function recurseCopyFilesAndFolders(folder, searchName, destFolder)
    for file in lfs.dir(folder) do
        if file ~= "." and file ~= ".." then
            ...
            if attr.mode == "directory" and string.find(file, searchName) then
            ...
            elseif attr.mode == "file" and string.find(file, searchName) then
                copyFileOrDir(srcPath, destPath)
            end
        end
    end
...
local function processConfigItem(configLine)
    ...
    local docDestinationFolder = rootDestinationFolder .. "\\" .. targetFolderName .. "\\角色配置"
    createDirectory(docDestinationFolder)

    for _, searchName in ipairs(searchNames) do
        print("处理搜索名称: " .. searchName)
        recurseCopyFilesAndFolders(sourceFolder, searchName, docDestinationFolder)
    end
end
