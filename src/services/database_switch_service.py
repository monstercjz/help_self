...
class DatabaseSwitchService:
    ...
    def switch_database(
        ...
        Args:
            ...
            db_service_class: 目标数据库服务的类 (e.g., MemoDatabaseService)。
            config_service: 用于读写配置的服务。
            config_section: config.ini 中的区段名。
            config_key: config.ini 中的键名。

        Returns:
            一个新的、验证通过的数据库服务实例，如果用户取消或操作失败则返回 None。
        """
        ...
        try:
            new_db_service = db_service_class(new_path)
            if not new_db_service.validate_database_schema():
                raise ValueError(f"所选数据库文件不符合 {db_service_class.__name__} 的结构要求或无写入权限。")
            ...
            logging.info(f"Successfully switched {config_section} database to: {new_path}")
            
            return new_db_service
